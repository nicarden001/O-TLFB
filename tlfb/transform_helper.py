...
os.environ['TLFB_TOKEN'] = 'B3489964B9B65FABF64E0CEA9FA073F3'


def transform_data(subid, timepoint, cohort, data, with_header=True):
    dk = '9678365400123890'
    ...
    pid_number = get_redcap_pid(subid, last_date)
...
def get_redcap_pid(subject_id, last_date):
     random_submission_number = random.randint(1, 9999)
     return f"{subject_id}-{last_date.strftime('%Y-%m-%d')}--{str(random_submission_number).zfill(4)}"
